...
void DateTime::computeGregorian(double julianDay)
{
	...
	ASSERT_DBG(_month >= 1 && _month <= 12);
	ASSERT_DBG(_day >= 1 && _day <= daysOfMonth(_year, _month));
	ASSERT_DBG(_hour >= 0 && _hour <= 23);
	ASSERT_DBG(_minute >= 0 && _minute <= 59);
	ASSERT_DBG(_second >= 0 && _second <= 59);
	ASSERT_DBG(_millisecond >= 0 && _millisecond <= 999);
	ASSERT_DBG(_microsecond >= 0 && _microsecond <= 999);
}
